...
Статус: численная модель и live AI реализованы. OpenAI выбирает ID проверенных фактов и рекомендаций; backend формирует текст и рассчитывает альтернативы. Без ключа или при ошибке доступен явный fallback. Публичного деплоя и сохранения сценариев нет.
...
| `ai/` | Live Responses API, Structured Outputs, проверка фактов, кэш и fallback; импортирует contracts | LEAD |
...
1. Эксплуатация live AI: настроить секрет окружения и подтвердить `mode=live`, `source=provider` на целевом сервере; health не заменяет реальный запрос.
