...
**Статус:** интегрированы `main` (`de14236`, переключатель RU / ҚАЗ / EN) и графики FRONTEND (`ffbc4c5`). Работают каталог, валидация, Score, API, dashboard, применение рекомендаций и сохранение одного сценария для локального сравнения. Несколько подписей новых графиков требуют дополнения словаря FRONTEND; серверный разбор сохраняется на русском. Живой AI в рабочем Docker подтверждён 23.09.2026: `live/provider`, без причины fallback. Для повторения нужен действующий ключ в окружении backend. Публичный деплой и общее хранилище команд пока не выполнены. Фактические проверки — в [статусе деплоя](docs/deployment-status.md).
