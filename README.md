...
**Статус:** каталог, валидация, расчёт Score, HTTP API и React-интерфейс работают локально. Из `main` интегрирован live AI LEAD: OpenAI Responses API, проверенные факты и альтернативные сценарии, таймаут и явный fallback. Для живого ответа нужен действующий API-ключ в окружении запущенного backend. Публичный деплой и сохранение сценариев пока не выполнены. Фактические проверки — в [статусе деплоя](docs/deployment-status.md).
